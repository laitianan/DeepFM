...
class DeepFM(nn.Module):
    ...
    def __init__(self, feature_sizes, embedding_size=4,
        ...
        for embedding in self.fm_first_order_embeddings:
            # 使用均值为0，标准差为0.01的正态分布初始化
            init.normal_(embedding.weight, mean=0.0, std=0.01)

        for embedding in self.fm_second_order_embeddings:
            # 使用均值为0，标准差为0.01的正态分布初始化
            init.normal_(embedding.weight, mean=0.0, std=0.01)
        """
            init deep part
        ...
        for epoch in range(epochs):
            for t, (xi, xv, y) in enumerate(loader_train):
                ...
                for param in model.parameters():
                    l2_reg += torch.norm(param)  # 或者使用 torch.sum(param**2)
